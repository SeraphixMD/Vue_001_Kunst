...
3. **cva for all variants.** `@kunst/ui/require-cva-for-variants` enforces that components in `rulesConfig.variants.required` (Button, Input, Dialog, Avatar, Badge) define their variants in a sibling `{Name}.variants.ts` file via `class-variance-authority`. Hand-rolled class-string concatenation in SFCs is forbidden.
...
6. **Scenarios for all components.** `@kunst/ui/require-scenario-for-component` requires every component in `rulesConfig.testing.requireScenarioFor` (14 components) to appear in `apps/sink/src/scenario-manifest.ts`.
